...
public class OrderRefundMoneyActivity extends BaseActivity {
    ...
    private void requestDetails() {
        ...
        OkHttpUtil.postAsyn(this,ConstantUrl.URL_SINGLE_GOOD_REFUND, new BeanCallback<String>() {
            @Override
    ...
    private void refundCommit() {
        ...
        if (inputMaxPrice.length() == 0 || new BigDecimal(inputMaxPrice).compareTo(new BigDecimal(0)) == 0 || new BigDecimal(inputMaxPrice).compareTo(ordersGoodsVo.getGoodsPayAmount()) == 1) {
            TToast.showShort(context, "退款金额不能为空，或不能超过可退金额");
            return;
        }
        ...
        OkHttpUtil.postAsyn(this,ConstantUrl.URL_SINGLE_GOOD_FEFUND_SAVE, new BeanCallback<String>() {
            @Override
            public void response(String data) {
