...
public class OkHttpUtil {
    ...
    public static void getAsyn(Context cxt,String url, ResultCallback callback) {
        checkInternetState(cxt,url);
    ...
    public static void postAsyn(Context cxt ,String url, final ResultCallback callback, Param... params) {
        checkInternetState(cxt,url,params);
        getInstance()._postAsyn(url, callback, params);
    }


    public static void postAsyn(Context cxt, String url, final ResultCallback callback, Map<String, String> params) {
        checkInternetStateM(cxt, url, params);
        getInstance()._postAsyn(url, callback, params);
    }
    ...
    public static void displayImage(Context ctx,final ImageView view, String url) {
        checkInternetState(ctx,url);
        getInstance()._displayImage(view, url, -1);
    }

    public static void downloadAsyn(Context ctx ,String url, String destDir, ResultCallback callback) {
        checkInternetState(ctx,url);
    ...
    private static void checkInternetState(Context ctx,String url,Param... params){
        ...
        LogHelper.i(TAG, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        LogHelper.i(TAG, ">> 请求URL-->" + url);
        if(null != params){
            for (Param param: params) {
                LogHelper.i(TAG, ">> 请求参数-->" + param.toString());
            }
    ...
    private static void checkInternetStateM(Context ctx,String url,Map<String,String> params){
        ...
        if(null != params){
            for (String key : params.keySet()) {
                LogHelper.i(TAG, ">> 请求参数-->" + "key = " + key + ", value = " + params.get(key));
            }
