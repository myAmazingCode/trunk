 ...
 * 订单商品投诉
 */
public class OrderComplaintGoodActivity extends BaseActivity {
    ...
    private List<ComplainSubject> complainsubjectlist;
    ...
    protected void setView() {
        setContentView(R.layout.activity_order_complaint_good);
    }
    ...
    private void requestDetails() {
        ...
        OkHttpUtil.postAsyn(this,ConstantUrl.URL_MEMBER_COMPLAIN_ADD, new BeanCallback<String>() {
            @Override
            public void response(String data) {
                ...
                complainsubjectlist = JsonUtil.toBean(data, "complainSubjectList", new TypeToken<ArrayList<ComplainSubject>>() {
                }.getType());
                bindData();
            }
        }, params);
    ...
    private void bindData() {
        ...
        tvReason.setText(complainsubjectlist.get(0).getTitle());
        selectedReasonId = complainsubjectlist.get(0).getSubjectId();
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.llReason:
                ...
                dialog.setView(v);
                dialog.show();
                listView.setAdapter(new CommonAdapter<ComplainSubject>(context, complainsubjectlist, R.layout.follow_botton_dialog_item) {
                    @Override
                    public void convert(ViewHolder holder, final ComplainSubject reasonItem) {
                        Button btnSortView = holder.getView(R.id.btnSortView);
                        btnSortView.setText(reasonItem.getTitle());

                        btnSortView.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                selectedReasonId = reasonItem.getSubjectId();
                                tvReason.setText(reasonItem.getTitle());
                                dialog.dismiss();
                            }
                        });
                    }
...
//        }
        ...
        if (buyerMessage.length() == 0) {
            TToast.showShort(context, "请填写投诉内容");
            return;
        }
        ...
        params.put("ordersId", ordersId + "");
        params.put("ordersGoodsId", ordersGoodsId + "");
        params.put("subjectId", selectedReasonId + "");
        params.put("accuserImages", translateImage());
        params.put("accuserContent", buyerMessage);

        OkHttpUtil.postAsyn(this,ConstantUrl.URL_MEMBER_COMPLAIN_ADD_SAVE, new BeanCallback<String>() {
            @Override
            public void response(String data) {
                Intent intent = new Intent(activity, OrderRefundAndReturnListActivity.class);
                intent.putExtra("flag", "complaint");
